...
<template>
  <section
    class="root flex h-screen w-screen items-center justify-center overflow-hidden bg-[var(--color-bg)] text-center text-[var(--color-text)] transition-all"
  >
    <h1 class="sr-only">
      {{ title }}
    </h1>
    ...
    >
      <Icon v-if="isLoading" name="eos-icons:three-dots-loading" class="text-[4rem]" />
      <template v-else>
        ...
        <span class="divider mb-6 inline-flex w-full items-center gap-2">
          <Icon name="hugeicons:quote-down" class="text-lg" />
        </span>
        <button
          aria-label="Get new advice"
          :disabled="isFetching"
          class="refresh-button absolute bottom-0 left-1/2 inline-flex -translate-x-1/2 translate-y-1/2 appearance-none items-center justify-center rounded-full bg-[var(--color-highlight)] p-3 text-center text-[var(--color-bg)] transition-all disabled:cursor-progress disabled:grayscale"
          @click="refreshAdvice"
        >
